...
(define-private (pox-delegate-stx-and-stack (amount-ustx uint) (pox-addr {version: (buff 1), hashbytes: (buff 20)}) (locking-period uint))
  (let ((ignore-result-revoke (contract-call? 'ST000000000000000000002AMW42H.pox revoke-delegate-stx))
        ...
        (stacker tx-sender))
      (match (contract-call? 'ST000000000000000000002AMW42H.pox delegate-stx amount-ustx accnt none none)
        success
            (match (as-contract (contract-call? 'ST000000000000000000002AMW42H.pox delegate-stack-stx stacker amount-ustx pox-addr start-block-ht locking-period))
...
(define-public (stack-aggregation-commit (pox-addr {version: (buff 1), hashbytes: (buff 20)}) (reward-cycle uint))
  (begin
    ...
    (match (as-contract (contract-call? 'ST000000000000000000002AMW42H.pox stack-aggregation-commit pox-addr reward-cycle))
      success (ok success)
      error (err (to-uint error)))))
...
(define-public (allow-contract-caller (this-contract principal))
  (begin
    ...
    (match (as-contract (contract-call? 'ST000000000000000000002AMW42H.pox allow-contract-caller this-contract none))
      success (ok true)
      error (err (to-uint error)))))
...
(define-read-only (burn-height-to-reward-cycle (height uint))
    (let ((pox-info (unwrap! (contract-call? 'ST000000000000000000002AMW42H.pox get-pox-info) u0)))
