...
(define-non-fungible-token b-22 uint)
...
(define-constant time-limit u712125) ;; add 4200 blocks
  ...
  (let
    ((id (+ u1 (var-get last-id))))
      ...
      (match (pox-delegate-stx-and-stack amount-ustx until-burn-ht)
        success-pox
            (match (nft-mint? b-22 id stacker)
              success-mint
                (begin
...
(define-read-only (nft-details (nft-id uint))
  (ok {stacked-ustx: (unwrap! (unwrap! (get stacked-ustx (map-get? meta nft-id)) err-invalid-asset-id) err-invalid-asset-id),
        owner: (unwrap! (nft-get-owner? b-22 nft-id) err-no-asset-owner)}))
...
(define-read-only (get-token-uri (id uint))
  (if (unwrap-panic (as-contract (contract-call? .bitcoin-day get-btc-time)))
    (ok (some "https://cloudflare-ipfs.com/ipfs/bafkreihkheacs722botylziw5l37cm3ewqxgxsz3tmurzm6cr6reaq3biq"))
    (ok (some "https://cloudflare-ipfs.com/ipfs/bafkreibhmggfrkx3xqbld5hydvufpyii7mekjdhlsoe65kfijsixftvbqi"))))
