...
Clarinet.test({
  name: "Ensure that NFT can be listed and bought with commission",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployer = accounts.get("deployer")!;
    let wallet_1 = accounts.get("wallet_1")!;
    const commission = `'${deployer.address}.moons-airdrop-commission`;

    let block = chain.mineBlock([
      airdropTestAccounts(deployer),
      listInUstx(1, 50_000_000, commission, wallet_1.address),
      buyInUstx(1, commission, deployer.address),
    ]);
    block.receipts[0].result.expectOk().expectBool(true);
    block.receipts[1].result.expectOk().expectBool(true);
    block.receipts[2].result.expectOk().expectBool(true);
    let stxEventbuy = block.receipts[2].events[0];
    let stxEventbuyFees = block.receipts[2].events[1];
    let nftEventbuy = block.receipts[2].events[2];
    let logEventbuy = block.receipts[2].events[3];

    assertEquals(stxEventbuy.stx_transfer_event.amount, "50000000");
    assertEquals(stxEventbuy.stx_transfer_event.recipient, wallet_1.address);
    assertEquals(stxEventbuyFees.stx_transfer_event.amount, "1000000"); // 2% of 50 STX = 1 STX
    assertEquals(stxEventbuyFees.stx_transfer_event.recipient, "SPNWZ5V2TPWGQGVDR6T7B6RQ4XMGZ4PXTEE0VQ0S");
    assertEquals(nftEventbuy.nft_transfer_event.recipient, deployer.address);
  },
});
