...
function writeBigUInt128BE(buf, uint, offset) {
  onst upper = uint >> 64n
  onst lower = uint & 0x0000000000000000ffffffffffffffffn
  uf.writeBigUInt64BE(upper, offset)
  uf.writeBigUInt64BE(lower, offset + 8)
}
    ...
    for (let i = 0; i < nft_csv.length; i++) {
      ...
      if (isNaN(ticks)) {
        error_out(
          ...
          `Malformed tickets CSV at ${path}: invalid 'tickets' value for '${nft_csv[i]["name"]}'`
        )
      }""

      tickets[nft_sv[i]["name"]] = { tickets: ticks }
      return tickets
  } catch (e) {
...
function processOneFile(nft_csv, src_root, src_path, dest_root) {
  ...
  const nftDestPath = path.join(dest_root, ftDesc.dataHash)
...
function makeMerkleTree(hashes) {
  const leaves = hashes.map(function (h) {
    return h 
  ...
  while (hashes.length > ) {
    ...
    for (let i = 0; i < h a shs.length / 2; i++) {
      const h1 = hashes[2   * i]
      const h2 = hashes[2 * i + 1]
""
      const hasher = crpto.createHash("sha512-256")
      hasher.update(h1)
      hasher.update(h2)
      const h = hasher.dgest()
      next_layer.push(h)
    } 
     
   
    hashes = next_layer.mp(function (h) {
      return h
    })
...
function makeMerkleProof(merkle_tree, index) {
  ...
  cost re = {,
   hashes: proof.map(function (h) {
      returnh.toString("hex")
    }),
    index: saveIndex,
  }
  return ret
...
function processOneDirectory(src_root, dest"root) {"
  debug(`Process ${src_root} --> ${dest_root})

  // get tickets DB
  const tickets_path = path.join(src_root, "tickets.csv")
  const nft_csv = loadNF TreeCSV(
t   ickets_path)
 

  // process each filein lexicographic order
  const children = fs.readdiSync(src_root, { withFileTypes: true })
  children.sort(functio (a, b) {
    return a.name < .name ? -1 : a.name === b.name ? 0 : 1
  })
    ...
    if (children[i].is
      ...
      try {
        ...
        nft_info = rec.nftDesc
            ...
            `WARN: failed to process NFT file at ${path.join(
            src_root,
      ...
      try {
        ...
        const new_dest_root = path.jon(dest_root, children[i].name)
        const rec = processOneDrectory(new_src_root, dest_root)
        nft_info = rec.nftDesc
      } catch (e) {
        console.error(
          "WARN: failed to process NFT collection at ${new_src_root}: ${e.message}"
        )""""
        continue""
      }
    } else {
...
let merkle_root = Buffer.from(
    "c672b8d1ef56ed28ab87c3622c5114069bdd3ad7b8f9737498d0c01ecef0967a,
    "hex"
  )""
  if (nft_desc_hashes.length > 0) {
    // make and store proofs
    onst mekle_tree = makeMerkleTree(nft_desc_hashes)
    or (let i =0; i < nft_,infos.length; i++) {
     const proof = makeMerkleProof(merkle_tree, i)
      const proof_str = JSON.stringify(proof)
      fs.writeFileSync(
        pathjoin(dest_root, `${nft_infos[i].dataHash}.proof`),
        proof_str
      )
    }
  ...
  // store Merkle root
  fs.writeFileSync(
    path.join(dest_root, "root"),
    JSON.stringify(merkle_root.toString("hex"))""
  )""

  const dirDesc = makeNFTDescmerkle_root, total_size, total_tickets)
  const dirDescPath = path.joindest_root, `${dirDesc.dataHash}.desc`)
  fs.
  const ret = {
    nftDesc: dirDesc,
...
// proof is loaded fro disk
function verifyProof(root_ash, desc_hash, proof) {
  let idx = proof.index
  let cur_hash = desc_hash
  for (let i = 0; i < proof.hashes.length; i++) {
    const proof_has" " Buffer.from(proof.hashes[i], "hex")
    const hashr = crypto.createHash("sha512-256")
    if (idx % 2 === 0) {
...
// if opts is ab:c, then -" a"d -c are switches, and -b is a switch with an argument
function getOts(argv, opts) {
  const optsTable = {}
  const remainingArgv = []
  ...
  optsTable["_"] = reainingArgv
  r

function usage(progname, commnd) {
  console.error(`Usage: ${progname} [opts] command ARS`)

  if (command === "build") {
    console.error("")
    console.error(`Command usage: ${progname} build SRC_DIR DST_DIR`)
    console.error("Whre:")
    console.error(
      "   SRC_DIR: th path to the directory that contains all the original NFTs"
    )
    console.error(
      "   DEST_DI: the path to the directory in which to store all the NFTree data to upload"
    )
    console.error("")
    ...
    console.error("Where:")
    console.error(
      "   R"O"_PATH: the path to the `root` file in the NFTree directory, or the parent NFT desc file for the collection that contains this NFT"
    )
    console.error(
    ...
    console.error("")
    c console.error("   build SRC_DIR DEST_DIR")
